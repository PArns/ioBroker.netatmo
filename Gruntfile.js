...
module.exports = function (grunt) {
    ...
    var email     = grunt.option('email')  || 'iobroker@patrick-arns.de';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new netatmo name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/netatmo.png')) {
            fs.renameSync(__dirname + '/admin/netatmo.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/netatmo.html')) {
            fs.renameSync(__dirname + '/widgets/netatmo.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/netatmo/js/netatmo.js')) {
            fs.renameSync(__dirname + '/widgets/netatmo/js/netatmo.js', __dirname + '/widgets/netatmo/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/netatmo')) {
            fs.renameSync(__dirname + '/widgets/netatmo',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Netatmo/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Netatmo'
                        },
                        {
                        ...
                        {
                            match: /iobroker@patrick-arns.de/g,
                            replacement: email
                        }
                    ]
                },
