module.exports = function (myapi, myadapter) {
    ...
    this.init = function () {
        ...
        adapter.log.info(`Doorbell: Registering realtime events with iot instance`);
    ...
    async function onSocketAlert(data) {
        ...
        if (data) {
            ...
            if (DoorbellEvents.includes(data.event_type)) {
                ...
                await adapter.setStateAsync(`${devicePath}ring`, {
                    val: active,
                    ack: true
                });
                await adapter.setStateAsync(`${devicePath}${data.event_type}.snapshot_url`, { val: data.snapshot_url, ack: true });
                ...
                await adapter.setStateAsync(`${deviceEventPath}snapshot_id`, { val: null, ack: true });
                await adapter.setStateAsync(`${deviceEventPath}snapshot_key`, { val: null, ack: true });
                await adapter.setStateAsync(`${deviceEventPath}snapshot_url`, { val: data.snapshot_url, ack: true });
                await adapter.setStateAsync(`${deviceEventPath}vignette_id`, { val: null, ack: true });
                await adapter.setStateAsync(`${deviceEventPath}vignette_key`, { val: null, ack: true });
    ...
    */
    ...
    async function handleDoorbell(aDoorbell, aHome) {
        ...
        await adapter.extendOrSetObjectNotExistsAsync(infoPath, {
            type: 'channel',
            common: {
                name: `${aDoorbell.name || aDoorbell.id} Info`,
            },
            native: {
            }
        });
        ...
        for (const e of DoorbellEvents) {
            ...
            await adapter.extendOrSetObjectNotExistsAsync(`${fullPath}.${e}.snapshot_url`, {
                type: 'state',
                common: {
                    name: 'snapshot_url',
                    type: 'string',
        ...
        await adapter.extendOrSetObjectNotExistsAsync(`${fullPath}.ring`, {
            type: 'state',
            common: {
                name: 'active',
                type: 'boolean',
