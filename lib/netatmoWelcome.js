module.exports = function (myapi, myadapter) {
    ...
    this.setAway = function (data, callback) {
        if (data && data.homeId) {
            api.setPersonsAway(data.homeId, data.personsId, (err, res) => {
                err && adapter.log.error(`Error on setPersonsAway: ${err} ${res}`);
                callback && callback(err,res);
            });
        } else {
            homeIds.forEach(aHomeId =>
                api.setPersonsAway(aHomeId, data ? data.personsId : null, (err, res) => {
                    err && adapter.log.error(`Error on setPersonsAway: ${err} ${res}`);
                    callback && callback(err,res);
                })
            );
    ...
    this.setHome = function (data, callback) {
        if (data && data.homeId) {
            api.setPersonsHome(data.homeId, data.personsId, (err, res) => {
                err && adapter.log.error(`Error on setPersonsAway: ${err} ${res}`);
                callback && callback(err,res);
            });
        } else {
    ...
    this.requestUpdateIndoorCamera = async function () {
        const cameras = ['NACamera', 'NOC'];
        for (const type of cameras) {
            await new Promise(resolve => {
                api.homedataExtended({
                    gateway_types: type //['NACamera', 'NOC'], // 'NACamDoorTag', 'NIS' ?? no cameras
                }, async (err, data) => {
                    if (err !== null) {
