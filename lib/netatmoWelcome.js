module.exports = function (myapi, myadapter) {
    ...
    async function handleHome(aHome) {
        ...
        if (aHome.cameras) {
            for (const aCamera of aHome.cameras) {
    ...
    async function handlePerson(aPerson, aParent) {
        ...
        aPersonName = aPersonName.replace(adapter.FORBIDDEN_CHARS, '_').replace(/\s|\./g, '_');
                ...
                for (const aEventId in objEvents) {
                    ...
                    adapter.getState(aEventId + '.time', async (errTime, stateTime) => {
                        if (errTime) {
                            ...
                            adapter.log.debug(`Cleanup: Check time for ${aEventId}: (${stateTime.val}) ${cleanupDate} > ${eventDate}`);
        ...
        adapter.getForeignObjects(`${adapter.namespace}.${home}.Persons.Unknown.*`, 'channel', (errPerson, objPerson) => {
            if (errPerson) {
                ...
                for (const aPersonId in objPerson) {
                    adapter.getState(aPersonId + '.last_seen', (errTime, objTime) => {
                        if (errTime) {
                            adapter.log.error(errTime);
                        } else if (objTime) {
                            for (const aTimeId in objTime) {
                                const personDate = Date.parse(objTime[aTimeId] ? objTime[aTimeId].val : 0);
